        ...
        <div style="width: 100%;">
          <div class="item_box" style="margin-top: 0px;">
            <div class="phone_item_box">
              ...
              <div class="flex_row form_row">
                <div class="form_label">文档</div>
                <div class="flex1 form_item">
                  <FormItem prop="books">
                    <uploader
                      upType="file"
                      v-model="formValidate.books"
                      upload-url="/backstageBaseDateC/uploadFile"
                      :base-url="baseUrl"
                      :format="['txt']"
                      :on-success="handleSuccessUpload"
                      :max-size="20480000"
                      over-max-size-message="文件不能超过2000M"
                      :num="1"
                      over-num-message="文件只能上传一个"
                      :fileType="5"
                      form-ref-name="formValidate"
                      ref="uploadFile"
                      delete-url="backstageBaseDateC/removeFile"
  ...
  data() {
    return {
      ...
      ruleValidate: {
        ...
        // pic: [{ required: true, type: "array", message: "请选择封面", trigger: "change" }],
        books: [{ required: true, type: "array", message: "请选择文档", trigger: "change" }],
      },
    } 
  },
  //加载组件时发出请求
  ...
  methods: {
    ...
    fnGetProductDtl(params) {
      axios.post("backstageC/searchArticleDetail", { id: params.row.id })
        .then(res => {
          ...
          // })
          this.$nextTick(() => {
            this.$refs.uploadFile.setFiletoRef()
    ...
    resetFields() {
      ...
      if (!this.modal_loading) {
        ...
        this.$refs.uploadFile.clear(true)
      }
